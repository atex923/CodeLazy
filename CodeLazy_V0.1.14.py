...
def fallback_sync_file_path() -> Path:
    return app_folder() / SYNC_FILE_NAME


def resolve_sync_export_target(target: Path, fallback: Path | None = None) -> Path:
    requested = Path(target).expanduser()
    fallback_path = Path(fallback).expanduser() if fallback is not None else fallback_sync_file_path()
    if os.name != "nt" and requested == sync_file_path(DEFAULT_SYNC_FOLDER):
...
def database_export_path(path: Path) -> Path:
    selected = Path(path).expanduser()
    return selected if selected.suffix else selected.with_suffix(".json")
...
class DataStore:
    ...
    def export_sync_file(self, target: Path, fallback: Path | None = None) -> tuple[bool, str]:
        target = resolve_sync_export_target(target, fallback)
        return self.export_database_file(target)

    def export_database_file(self, target: Path) -> tuple[bool, str]:
...
class AppSettings:
    ...
    def load(self) -> None:
        ...
        if isinstance(raw, dict):
            ...
            self.last_database_folder = str(raw.get("last_database_folder", ""))
            ...
            json.dumps(
                {
                    ...
                    "last_database_folder": self.last_database_folder,
                },
                ensure_ascii=False,
                indent=2,
            ),
    ...
    def remember_database_file(self, path: Path) -> None:
        self.last_database_folder = str(Path(path).expanduser().parent)
        self.save()
    ...
    def database_dialog_folder(self) -> Path:
        if self.last_database_folder:
            folder = Path(self.last_database_folder).expanduser()
            if folder.exists():
                return folder
        return app_folder()
...
class MainWindow(QMainWindow):
    ...
    def build_toolbar(self) -> QHBoxLayout:
        ...
        self.export_database_button = QPushButton("另存資料庫")
        ...
        self.export_database_button.clicked.connect(self.save_database_as)
        ...
        row.addWidget(self.export_database_button)
    ...
    def save_database_as(self) -> None:
        if self.dirty:
            ...
            if self.dirty:
                return
        suggested = self.settings.database_dialog_folder() / (
            f"CodeLazy_data_{datetime.now():%Y%m%d_%H%M%S}.json"
        )
        ...
        if not chosen:
            return
        target = database_export_path(Path(chosen))
        ok, detail = self.store.export_database_file(target)
        if not ok:
            QMessageBox.critical(self, "另存失敗", f"無法另存資料庫：\n{detail}")
            return
        self.settings.remember_database_file(target)
        self.status_label.setText(f"資料庫已另存：{target.name}")
        QMessageBox.information(self, "另存資料庫", f"資料庫已另存至：\n{target}")
