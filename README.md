...
- macOS 玻璃感無邊框介面，保留紅黃綠視窗控制鈕。
- 開發項目自動連號；刪除或同步後會重新整理項次。
- 版號以 `V0.1.5` 這類三段式格式管理，每段可用 `+ / -` 調整。
- 儲存後鎖定基本資料與說明，保留最新版號、修改方式、備註可快速更新。
...
- V0.1.5 新增 `CODELAZY_SYNC_FOLDER` 環境變數，可覆寫同步資料夾，方便不同磁碟代號或測試環境使用。
...
```text
Nuitka_Output\CodeLazy_V0.1.5.exe
```
...
GitHub root 只保留目前最新版 V0.1.5 的程式與文件。舊版完整來源收在 `history/Vx.y.z/`，索引見 `history/README.md`。

使用者資料檔 `CodeLazy_data.json` 與同步檔 `CodeLazy_sync.txt` 不納入公開倉庫。
