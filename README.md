...
- macOS 玻璃感無邊框介面，視窗控制鈕位於右側。
- 開發項目自動連號；刪除或同步後會重新整理項次。
- 版號以 `V0.1.6` 這類三段式格式管理，每段可用 `+ / -` 調整。
...
- V0.1.6 會記住最後一次選取的同步紀錄檔位置，並支援拖曳 `.txt` / `.json` 同步檔載入。
...
```text
Nuitka_Output\CodeLazy_V0.1.6.exe
```
...
GitHub root 只保留目前最新版 V0.1.6 的程式與文件。舊版完整來源收在 `history/Vx.y.z/`，索引見 `history/README.md`。
