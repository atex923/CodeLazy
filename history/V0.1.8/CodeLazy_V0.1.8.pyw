...
class ProjectTable(QTableWidget):
    ...
    def dropEvent(self, event):
        ...
        if not selected or event.source() is not self:
            super().dropEvent(event)
            return
        record_id = str(selected[0].data(Qt.ItemDataRole.UserRole))
        target_row = self.rowAt(event.position().toPoint().y())
        if target_row < 0:
            target_row = self.rowCount()
        QTimer.singleShot(0, lambda: self.rowsReordered.emit(record_id, target_row))
        event.acceptProposedAction()
